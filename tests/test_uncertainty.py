...
class TestMonteCarloCore(unittest.TestCase):
    ...
    def test_estimate_emissions_distribution_basic(self):
        ...
        self.assertTrue(all(s >= 0 for s in samples))  # All emissions should be positive
...
class TestUncertaintyAwareEmissions(unittest.TestCase):
    ...
    @patch('codecarbon.core.uncertainty_emissions.UncertaintyAwareEmissions.get_private_infra_emissions')
    def test_get_private_infra_emissions_with_uncertainty(self, mock_base_method):
        ...
        emissions_kg, uncertainty = self.emissions_calc.get_private_infra_emissions_with_uncertainty(
            energy, self.geo
        )
    ...
    def test_uncertainty_disabled(self):
        ...
        with patch.object(self.emissions_calc, 'get_private_infra_emissions', return_value=0.3):
            energy = Energy.from_kWh(1.0)
            emissions_kg, uncertainty = self.emissions_calc.get_private_infra_emissions_with_uncertainty(
                energy, self.geo
            )
